...
class AppState:
    """
    Centralized global state for the Flet application.
    ...
    This state is shared across all views and components using
    Flet's built-in state management.
    """
    ...
    def get_flet_theme_mode(self) -> ft.ThemeMode:
        """
        Get the Flet ThemeMode enum value.

        Returns:
            ft.ThemeMode.LIGHT or ft.ThemeMode.DARK
        """
        return ft.ThemeMode.LIGHT if self.theme_mode else ft.ThemeMode.DARK
